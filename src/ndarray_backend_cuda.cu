...
__device__ void gid_idx(size_t gid, CudaVec shape, CudaVec* idx) {
  int32_t prod = 1;
...
__device__ size_t cuda_vector_dot_prod(CudaVec a, CudaVec b, size_t extra) {
...
__device__ size_t compact_idx(size_t gid, CudaVec shape, CudaVec strides, size_t offset) {
  CudaVec idx = {shape.size};
  gid_idx(gid, shape, &idx);
  return cuda_vector_dot_prod(strides, idx, offset);
}

__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec strides, size_t offset) {
   /**
...
//    */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) 
    out[gid] =a[compact_idx(gid, shape, strides, offset)];
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) 
    out[compact_idx(gid, shape, strides, offset)]= a[gid];
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape, CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) 
    out[compact_idx(gid, shape, strides, offset)] = val;
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  dim3 m_grid((M + 16 - 1) / 16, (P + 16 - 1) / 16, 1);
  dim3 m_block(16, 16, 1);
  MatmulKernel<<<m_grid, m_block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
