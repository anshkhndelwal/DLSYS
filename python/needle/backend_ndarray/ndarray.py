...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        return NDArray.make(tuple(np.array(self._shape)[list(new_axes)]), tuple(np.array(self._strides)[list(new_axes)]), self._device, self._handle)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        p_shape = [1] * (len(new_shape) - len(self.shape)) + list(self.shape)
        p_strides = [0] * (len(new_shape) - len(self.shape)) + list(self.strides)
        for i, shape in enumerate(p_shape):
          if shape != 1:
            assert(shape == new_shape[i])
        ...
        return NDArray.make(new_shape,tuple(p_strides),self._device,self._handle,self._offset).compact()
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        s = [-self.strides[a] if a in axes else self.strides[a] for a in range(len(self.shape))]
        idx = [self.shape[a] - 1 if a in axes else 0 for a in range(len(self.shape))]
        offset = 0
        for i in range(len(self.shape)):
            offset = offset + (idx[i]*self.strides[i])
        out = NDArray.make(self.shape,strides=s,device=self.device,handle=self._handle,offset=offset,).compact()
        return out
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        slices = []
        for pad_width in axes:
            start = pad_width[0]
            end = -pad_width[1] if pad_width[1] else None
            axis_slice = slice(start, end, 1)
